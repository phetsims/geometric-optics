...
* To ensure that rays pass through the optic, the optical object is always at least 40 cm from the optic, and never more than 100 cm from the optical axis. See `MIN_DISTANCE_FROM_OBJECT_TO_OPTIC` and `MAX_DISTANCE_FROM_OPTICAL_AXIS` in [GOConstants.ts](ttps://github.com/phetsims/geometric-optics/blob/master/js/common/GOConstants.ts).
